...
class Proxy {
    constructor(app){
        this._app = app;
        this._backends = this._app.backends;
    ...
    requestHandler(req, res){
        ...
        if(Object.keys(this._app.request.headers).length !== 0){
            for(let key in this._app.request.headers){
                req.headers[key] = this._app.request.headers[key];
            }
        }
    ...
    */
    ...
    [PRIVATE.ON_RESPONSE](res){
        if(Object.keys(this._app.response.headers).length !== 0){
            for(let key in this._app.response.headers){
                res.headers[key] = this._app.response.headers[key];
            }
        }
